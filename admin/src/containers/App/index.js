...
// added for - custom admin change route listener
import { useHistory } from "react-router-dom";

function App(props) {
  const getDataRef = useRef();
  const [{ isLoading, hasAdmin }, setState] = useState({
    isLoading: true,
    hasAdmin: false,
  });
  ...
  // custom admin change route listener
  const history = useHistory();
  useEffect(() => {
    window.addEventListener("message", (event) => {
      console.log("message recieved in app");
      const eventData = JSON.parse(event.data);
      const method = eventData.method;
      if (method === "changeRoute") {
        console.log("changeRoute", eventData.path);
        // .href will navigate in the iframe
